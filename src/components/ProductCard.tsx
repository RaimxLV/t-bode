...
const ProductImage = ({ src, alt }: { src: string; alt: string }) => {
  ...
  const srcSet = buildSrcSet(src, [256, 384, 512, 640, 800], 75) || undefined;
  return (
    <div className="relative w-full h-full bg-muted">
      ...
      <img
        src={optimized}
        srcSet={srcSet}
        sizes={srcSet ? "(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 320px" : undefined}
        alt={alt}
        className={`w-full h-full object-contain bg-white scale-125 transition-opacity duration-500 ${loaded ? "opacity-100" : "opacity-0"}`}
        loading="lazy"
        decoding="async"
