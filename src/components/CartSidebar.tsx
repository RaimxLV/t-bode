      ...
      <SheetContent className="flex flex-col w-full sm:max-w-md bg-background border-border">
        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 text-muted-foreground">
            ...
            <p className="font-body text-sm">Jūsu grozs ir tukšs</p>
          ...
          <>
                ...
                {items.map((item) => (
                  <div key={`${item.productId}-${item.size}-${item.color}`} className="flex gap-3">
                    <Link
                      ...
                      className="w-20 h-20 rounded-md overflow-hidden bg-card border border-border flex-shrink-0"
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <Link
                        to={`/product/${item.slug}`}
                        onClick={() => setIsOpen(false)}
                        className="font-body font-semibold text-sm truncate block hover:text-primary transition-colors"
                      >
                        {item.name}
                      </Link>
            ...
            <SheetFooter className="flex-col gap-3 pt-4 sm:flex-col">
              <div className="flex items-center justify-between w-full">
                <span className="font-body font-semibold">Kopā:</span>
