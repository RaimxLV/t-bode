    ...
    <section id="stores" className="py-16 md:py-24 bg-secondary/30">
      <div className="container mx-auto px-4">
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="mt-8 md:mt-12 bg-card rounded-lg overflow-hidden border border-border relative"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-5 md:p-8 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-2 md:mb-3">
                ...
                <h3 className="font-body font-bold text-base md:text-lg">{t("stores.officeTitle")}</h3>
              </div>
              <p className="text-muted-foreground font-body text-xs md:text-sm leading-relaxed mb-3 md:mb-4">
                {t("stores.officeDesc")}
              </p>
              <div className="space-y-1.5 md:space-y-2 text-xs md:text-sm text-muted-foreground font-body">
                <p className="font-medium text-foreground text-sm md:text-base">Braslas iela 29, Ieeja D, Rīga, LV-1084</p>
                <a href="tel:+37125486124" className="flex items-center gap-2 hover:text-foreground transition-colors">
                  <Phone className="w-3.5 h-3.5 md:w-4 md:h-4 text-primary flex-shrink-0" />+371 25 486 124
                </a>
                <a href="mailto:info@t-bode.lv" className="flex items-center gap-2 hover:text-foreground transition-colors">
                  <Mail className="w-3.5 h-3.5 md:w-4 md:h-4 text-primary flex-shrink-0" />info@t-bode.lv
                </a>
              </div>
              ...
              <div className="flex gap-2 md:gap-3 mt-4 md:mt-6">
                <a
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 md:gap-2 border border-border rounded-lg py-2.5 md:py-3 px-3 md:px-4 font-body font-bold text-xs md:text-sm hover:bg-secondary/50 transition-colors"
                >
                  <svg className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M20.54 6.63c.69 1.34 1.07 2.87 1.07 4.49 0 4.41-2.79 8.12-6.67 9.33-.25.08-.51-.12-.51-.38v-.71c0-.26.18-.49.43-.56 3.19-1.05 5.5-4.13 5.5-7.68 0-1.35-.32-2.62-.88-3.74a.544.544 0 0 1 .09-.6l.47-.47c.17-.17.45-.14.5.12zm-3.06-3.16c.17-.17.14-.45-.12-.5C16.02 2.28 14.55 1.9 12.93 1.9c-5.14 0-9.31 4.17-9.31 9.31 0 2.24.79 4.29 2.1 5.9.16.2.14.49-.05.65l-.47.47c-.17.17-.45.14-.6-.06A9.26 9.26 0 0 1 2.37 11.2C2.37 5.34 7.07.64 12.93.64c1.89 0 3.66.5 5.19 1.36.2.11.23.38.09.56l-.47.47c-.11.11-.28.15-.43.09-.01 0-.02-.01-.03-.01zM12 6a5 5 0 1 0 0 10 5 5 0 0 0 0-10zm0 8.5a3.5 3.5 0 1 1 0-7 3.5 3.5 0 0 1 0 7z"/></svg>
                  Waze
                </a>
                <a
                  ...
                  className="flex-1 flex items-center justify-center gap-1.5 md:gap-2 border border-border rounded-lg py-2.5 md:py-3 px-3 md:px-4 font-body font-bold text-xs md:text-sm hover:bg-secondary/50 transition-colors"
                >
                  <Navigation className="w-4 h-4 md:w-5 md:h-5 text-primary flex-shrink-0" />
                  Google Maps
                </a>
              </div>
            </div>
            <div
              className="h-52 md:h-auto min-h-[220px] md:min-h-[280px] relative group cursor-pointer z-0"
              onClick={(e) => {
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="text-center mt-6 md:mt-8 text-xs md:text-sm text-muted-foreground font-body flex items-center justify-center gap-2"
        >
